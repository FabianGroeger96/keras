...
class Dropout(Layer):
  ...
  def __init__(self, rate, noise_shape=None, seed=None, **kwargs):
    # Note that the constructor is annontated with
    # @no_automatic_dependency_tracking. This is mainly trying to skip the auto
    ...
    # that state into model.weights() and checkpoints for backward comapt reason
    # In the meantime, we still need to make them visible to SavedModel when
    # it is tracing the tf.function for the `call()`.
  ...
  def _list_extra_dependencies_for_serialization(self, serialization_cache):
    # This method is trying to expose the self._random_generator to SavedModel
    # only (not layer.weights and checkpoint).
    deps = super(Dropout, self)._list_extra_dependencies_for_serialization(
        serialization_cache)
